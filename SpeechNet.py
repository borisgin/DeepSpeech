...
def batch_norm(name,
    ...
    batch_mean, batch_var = tf.nn.moments(input, [0, 1, 2])
    output = tf.nn.batch_normalization(input, batch_mean, batch_var, beta, gamma, bn_epsilon)
...
def SpeechNet1(batch_x, seq_length,training):
    ...
    outputs = tf.nn.dropout(x=outputs, keep_prob = dropout_keep_prob)
...
def calculate_mean_edit_distance_and_loss(model_feeder, tower, training):
    ...
    logits, batch_seq_len = SpeechNet1(batch_x, batch_seq_len, training=training)
...
def create_inference_graph(batch_size=None, output_is_logits=False, use_new_decoder=False):
    ...
    logits,_ = SpeechNet1(input_tensor, tf.to_int64(seq_length) if FLAGS.use_seq_length else None)
