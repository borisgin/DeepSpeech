...
def create_inference_graph(batch_size=None, output_is_logits=False):
    ...
    if custom_op_module is not None:
        # Beam search decode the batch
        decoded, _ = decode_with_lm(logits, seq_len, merge_repeated=False, beam_width=FLAGS.beam_width)
    else:
        # Greedy decoding
        decoded, _ = decode_without_lm(logits, seq_len, merge_repeated=True)
